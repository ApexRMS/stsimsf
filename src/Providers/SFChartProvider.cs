    ...
    internal class SFChartProvider : ChartProvider
    {
        public override DataTable GetData(DataStore store, ChartDescriptor descriptor, DataSheet dataSheet)
        {
                ...
                descriptor.DatasheetName == Constants.DATASHEET_OUTPUT_FLOW_NAME)
            {
                ...
                if (
                    VarName == Constants.STOCK_GROUP_VAR_NAME ||
                    VarName == Constants.STOCK_GROUP_DENSITY_VAR_NAME ||
                    VarName == Constants.FLOW_GROUP_VAR_NAME ||
                    VarName == Constants.FLOW_GROUP_DENSITY_VAR_NAME)
                {
        ...
        private static void AddStockGroupChartVariables(Project project, SyncroSimLayoutItemCollection items)
        {
            ...
            if (ds.HasData())
            {
                ...
                //Density
                SyncroSimLayoutItem ItemDensity = new SyncroSimLayoutItem(Constants.STOCK_GROUP_DENSITY_VAR_NAME, "Density", false);
        ...
        private static void AddFlowGroupChartVariables(Project project, SyncroSimLayoutItemCollection items)
        {
            ...
            if (ds.HasData())
            {
                ...
                //Density
                SyncroSimLayoutItem ItemDensity = new SyncroSimLayoutItem(Constants.FLOW_GROUP_DENSITY_VAR_NAME, "Density", false);
        ...
        {
            Debug.Assert(
                variableName == Constants.STOCK_GROUP_VAR_NAME ||
                variableName == Constants.STOCK_GROUP_DENSITY_VAR_NAME ||
                variableName == Constants.FLOW_GROUP_VAR_NAME ||
